  ...
  case wisp.path_segments(req) {
    ...
    ["meals", meal_id] -> {
      ...
      case parsed_parameters {
        ...
        Error(Nil) ->
          error_message(meal_id <> " n'est pas un identifiant de repas valide")
      }
    }
    ...
    ["meals-generate"] -> {
      ...
      let meal_dates =
        ...
        |> result.map(fn(dates) {
          list.map(dates, fn(meal_date) { datetime.from_unix_milli(meal_date) })
        })
          ...
          case req.method {
            http.Delete -> {
              case recipe_adapter.delete(valid_recipe_id, ctx.connection) {
                Ok(_) -> string_tree.new() |> wisp.html_response(200)
                Error(_) -> error_message("L'effacement de la recette a échoué")
              }
            }
            _ -> wisp.method_not_allowed([])
          }
        ...
        Ok(#(valid_meal_id, valid_recipe_id)) -> {
          ...
          case meal, recipe {
            Ok(valid_meal), Ok(valid_recipe) -> {
              let updated_meal_recipes =
                meals.suggest(ctx, valid_meal, valid_recipe)
              case updated_meal_recipes {
                Ok(valid_recipes) ->
        ...
        Ok(path) -> {
          ...
          case recipe_routes.from_xml(file_content) {
            Ok(parsed_recipes) -> {
              ...
              let _ = recipe_adapter.bulk_insert(parsed_recipes, ctx.connection)
