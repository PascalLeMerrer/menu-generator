...
fn optional_int_decoder() -> decode.Decoder(option.Option(Int)) {
  ...
  use value_as_string <- decode.then(nested_string_decoder())
  value_as_string |> int.parse |> option.from_result |> decode.success
}

fn duration_decoder() -> decode.Decoder(option.Option(Int)) {
  // decode.then builds a decoder based on the one it is given as an argument
