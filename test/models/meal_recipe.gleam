...
pub fn decodes_meal_recipe_with_one_step_and_one_ingredient_test() {
  dynamic.from(#(image, "a unique ingredient", 42, "a unique step", title))
  |> meal_recipe.decode()
  |> should.equal(
    Ok(
...
pub fn decodes_meal_recipe_with_several_steps_and_ingredients_test() {
  dynamic.from(#(
    image,
...
pub fn decodes_real_meal_recipe_test() {
  ...
  let real_meal_id = 8_665_524_133
  let real_steps =
